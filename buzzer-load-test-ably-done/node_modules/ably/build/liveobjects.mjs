...
var ObjectId = class _ObjectId {
   ...
   */
  static async generateNonce(client) {
...
function decodeObjectOperationAction(action) {
  return operationActions[action] || "unknown";
}
var mapSemantics = ["lww"];
function encodeMapSemantics(semantics, client) {
...
function decodeMapSemantics(semantics) {
  var _a;
  return (_a = mapSemantics[semantics]) != null ? _a : "unknown";
}
function getObjectDataPrimitive(data) {
  var _a, _b, _c, _d;
  return (_d = (_c = (_b = (_a = data.boolean) != null ? _a : data.bytes) != null ? _b : data.number) != null ? _c : data.string) != null ? _d : data.json;
}
function primitiveToObjectData(value, client) {
...
function encode(message, utils, messageEncoding, encodeObjectDataFn) {
  ...
  if ((_e = (_d = (_c = message.object) == null ? void 0 : _c.createOp) == null ? void 0 : _d.mapCreate) == null ? void 0 : _e.entries) {
    result.object.createOp.mapCreate.entries = encodeMapEntries(
      message.object.createOp.mapCreate.entries,
      encodeObjectDataFn
    );
  }
  if ((_g = (_f = message.operation) == null ? void 0 : _f.mapCreate) == null ? void 0 : _g.entries) {
    result.operation.mapCreate.entries = encodeMapEntries(message.operation.mapCreate.entries, encodeObjectDataFn);
  }
  if ((_i = (_h = message.operation) == null ? void 0 : _h.mapSet) == null ? void 0 : _i.value) {
    result.operation.mapSet.value = encodeObjectData(message.operation.mapSet.value, encodeObjectDataFn);
  }
  if ((_l = (_k = (_j = message.operation) == null ? void 0 : _j.mapCreateWithObjectId) == null ? void 0 : _k._derivedFrom) == null ? void 0 : _l.entries) {
...
function encodePartialObjectOperationForWire(operation, client, format) {
...
function decodeWireObjectData(wireData, client, format) {
  try {
    if (wireData.objectId != null) {
      return { objectId: wireData.objectId };
    }
    if (wireData.bytes != null) {
      const decodedBytes = format === "msgpack" ? (
    ...
    if (wireData.json != null) {
      return { json: JSON.parse(wireData.json) };
    }
    ...
    return __spreadValues({}, wireData);
  } catch (error) {
    client.Logger.logAction(
      client.logger,
...
function toUserFacingObjectData(data) {
...
function toUserFacingMapEntry(entry) {
  return __spreadProps(__spreadValues({}, entry), {
    data: entry.data ? toUserFacingObjectData(entry.data) : void 0
  });
}
function toUserFacingObjectOperation(operation) {
  var _a, _b, _c, _d;
  const { mapSet: internalMapSet, mapRemove, counterInc, objectDelete, mapClear } = operation;
  const internalMapCreate = (_b = operation.mapCreate) != null ? _b : (_a = operation.mapCreateWithObjectId) == null ? void 0 : _a._derivedFrom;
  const counterCreate = (_d = operation.counterCreate) != null ? _d : (_c = operation.counterCreateWithObjectId) == null ? void 0 : _c._derivedFrom;
  let mapCreate;
  if (internalMapCreate) {
    mapCreate = __spreadProps(__spreadValues({}, internalMapCreate), {
  ...
  let mapSet;
  if (internalMapSet) {
    mapSet = __spreadProps(__spreadValues({}, internalMapSet), {
...
var ObjectMessage = class _ObjectMessage {
  ...
  encode() {
    const encodeObjectDataFn = (data) => {
      const encodedObjectData = __spreadProps(__spreadValues({}, data), {
        json: data.json != null ? JSON.stringify(data.json) : void 0
        // OD4c5, OD4d5
      });
...
var WireObjectMessage = class _WireObjectMessage {
  ...
  encodeForWire(format) {
    ...
    const encodeObjectDataFn = (data) => {
      if (data.bytes != null) {
        const result2 = this._messageEncoding.encodeDataForWire(data.bytes, null, format);
        return __spreadProps(__spreadValues({}, data), { bytes: result2.data });
      }
      return __spreadValues({}, data);
    };
    const result = encode(this, this._utils, this._messageEncoding, encodeObjectDataFn);
    if ((_a = result.operation) == null ? void 0 : _a.mapCreateWithObjectId) {
      delete result.operation.mapCreateWithObjectId._derivedFrom;
    }
    if ((_b = result.operation) == null ? void 0 : _b.counterCreateWithObjectId) {
  ...
  decode(client, format) {
    ...
    try {
      ...
      if ((_e = (_d = (_c = this.object) == null ? void 0 : _c.createOp) == null ? void 0 : _d.mapCreate) == null ? void 0 : _e.entries) {
      ...
      if ((_g = (_f = this.operation) == null ? void 0 : _f.mapCreate) == null ? void 0 : _g.entries) {
        result.operation.mapCreate.entries = this._decodeMapEntries(this.operation.mapCreate.entries, client, format);
      }
      if ((_i = (_h = this.operation) == null ? void 0 : _h.mapSet) == null ? void 0 : _i.value) {
  ...
  _getObjectOperationSize(operation) {
    var _a, _b, _c, _d;
    let size = 0;
    const mapCreate = (_b = operation.mapCreate) != null ? _b : (_a = operation.mapCreateWithObjectId) == null ? void 0 : _a._derivedFrom;
    if (mapCreate) {
  ...
  /** @spec MCR3 */
  _getMapCreateSize(mapCreate) {
    var _a;
    let size = 0;
    Object.entries((_a = mapCreate.entries) != null ? _a : {}).forEach(([key, entry]) => {
      var _a2;
      size += (_a2 = key == null ? void 0 : key.length) != null ? _a2 : 0;
      if (entry) {
        size += this._getMapEntrySize(entry);
  ...
  /** @spec MST3 */
  _getMapSetSize(mapSet) {
    var _a, _b;
    let size = 0;
    size += (_b = (_a = mapSet.key) == null ? void 0 : _a.length) != null ? _b : 0;
    if (mapSet.value) {
  ...
  /** @spec MRM3 */
  _getMapRemoveSize(mapRemove) {
    var _a;
    return (_a = mapRemove.key.length) != null ? _a : 0;
  }
  /** @spec CCR3 */
  _getCounterCreateSize(counterCreate) {
  ...
  _decodeMapEntries(mapEntries, client, format) {
    return Object.entries(mapEntries).reduce(
      (acc, v) => {
        const [key, entry] = v;
        const decodedData = entry.data ? decodeWireObjectData(entry.data, client, format) : void 0;
        acc[key] = __spreadProps(__spreadValues({}, entry), {
          data: decodedData
        });
        return acc;
...
var LiveCounterValueType = class _LiveCounterValueType {
  ...
  static async createCounterCreateMessage(realtimeObject, value) {
    ...
    const counterCreate = _LiveCounterValueType._getCounterCreate(count);
  ...
  static _getCounterCreate(count) {
    return {
...
var LiveObject = class {
   ...
   * @spec RTLO6
   */
  _calculateTombstonedAt(serialTimestamp, action, details) {
  ...
  _notifyPathSubscriptions(update) {
    ...
    for (const pathToThis of pathsToThis) {
      const preferenceOrderedCandidatePaths = [pathToThis];
...
var ObjectsPool = class {
  ...
  _onGCInterval() {
    ...
    for (const [objectId, obj] of this._pool.entries()) {
      if (objectId !== ROOT_OBJECT_ID && obj.isTombstoned() && Date.now() - obj.tombstonedAt() >= this._realtimeObject.gcGracePeriod) {
...
var DefaultPathObject = class _DefaultPathObject {
  ...
  *values() {
  ...
  async batch(fn) {
    ...
    const instance = new DefaultInstance(this._realtimeObject, resolved);
...
var PathObjectSubscriptionRegister = class {
   ...
   */
  notifyPathEvent(event) {
    var _a;
    for (const subscription of this._subscriptions.values()) {
      const chosenCoveredPath = event.preferenceOrderedCandidatePaths.find(
        (path) => this._subscriptionCoversPath(subscription, path)
      );
      if (chosenCoveredPath === void 0) {
        continue;
      }
      try {
   ...
   */
  _subscriptionCoversPath(subscription, eventPath) {
...
var SyncObjectsPool = class {
   ...
   * @spec RTO5f2a
   */
  _mergeMapSyncState(existingEntry, newObjectMessage) {
...
var RealtimeObject = class {
  constructor(channel) {
    ...
    this._appliedOnAckSerials = /* @__PURE__ */ new Set();
  ...
  handleObjectSyncMessages(objectMessages, syncChannelSerial) {
    ...
    this._syncObjectsPool.applyObjectSyncMessages(objectMessages);
    if (!syncCursor) {
      this._endSync();
    }
  }
  ...
  handleObjectMessages(objectMessages) {
    ...
    this._applyObjectMessages(objectMessages, "channel" /* channel */);
  }
  /**
   * @internal
   * @spec RTO4
   ...
   * @spec RTO20
   */
  async publishAndApply(objectMessages) {
    var _a, _b;
    ...
    const siteCode = (_a = this._channel.connectionManager.connectionDetails) == null ? void 0 : _a.siteCode;
    if (!siteCode) {
      this._client.Logger.logAction(
        this._client.logger,
        this._client.Logger.LOG_ERROR,
    ...
    if (!publishResult.serials || publishResult.serials.length !== objectMessages.length) {
      this._client.Logger.logAction(
        ...
        this._client.Logger.LOG_ERROR,
        "RealtimeObject.publishAndApply()",
        `operations will not be applied locally: PublishResult.serials has unexpected length (expected ${objectMessages.length}, got ${(_b = publishResult.serials) == null ? void 0 : _b.length}); channel=${this._channel.name}`
      );
      return;
    }
    ...
    for (let i = 0; i < objectMessages.length; i++) {
      ...
      syntheticMessages.push(
        ObjectMessage.fromValues(
          __spreadProps(__spreadValues({}, objectMessages[i]), {
            serial,
            // RTO20d2a
            siteCode
            // RTO20d2b
        ...
        const onChannelState = () => {
          ...
          reject(
            new this._client.ErrorInfo(
              ...
              92008,
              400,
              this._channel.errorReason || void 0
            )
          );
        };
        this._eventEmitterInternal.once("synced" /* synced */, onSynced);
    ...
    this._applyObjectMessages(syntheticMessages, "local" /* local */);
  }
  /**
   * @internal
   */
  ...
  _endSync() {
    this._applySync();
    this._applyObjectMessages(this._bufferedObjectOperations, "channel" /* channel */);
    this._bufferedObjectOperations = [];
    this._syncObjectsPool.clear();
  ...
  _applySync() {
    ...
    for (const [objectId, objectMessage] of this._syncObjectsPool.entries()) {
      ...
      if (existingObject) {
        const update = existingObject.overrideWithObjectState(objectMessage);
        existingObjectUpdates.push({ object: existingObject, update });
        continue;
      }
      let newObject;
      if ((_a = objectMessage.object) == null ? void 0 : _a.counter) {
        newObject = LiveCounter.fromObjectState(this, objectMessage);
      } else if ((_b = objectMessage.object) == null ? void 0 : _b.map) {
        newObject = LiveMap.fromObjectState(this, objectMessage);
      } else {
        this._client.Logger.logAction(
          this._client.logger,
          this._client.Logger.LOG_MAJOR,
  ...
  /** @spec RTO9 */
  _applyObjectMessages(objectMessages, source) {
    for (const objectMessage of objectMessages) {
      ...
      switch (objectOperation.action) {
        ...
        case 6 /* MAP_CLEAR */: {
          ...
          const applied = this._objectsPool.get(objectOperation.objectId).applyOperation(objectOperation, objectMessage, source);
          if (source === "local" /* local */ && applied && serial) {
...
var LiveCounter = class _LiveCounter extends LiveObject {
  ...
  _mergeInitialDataFromCreateOperation(objectOperation, msg) {
    var _a, _b;
    const counterCreate = (_b = objectOperation.counterCreate) != null ? _b : (_a = objectOperation.counterCreateWithObjectId) == null ? void 0 : _a._derivedFrom;
    const count = counterCreate == null ? void 0 : counterCreate.count;
    this._createOperationIsMerged = true;
    if (this._client.Utils.isNil(count)) {
      return { noop: true };
    }
    this._dataRef.data += count;
    return {
      update: { amount: count },
      objectMessage: msg,
...
var LiveMap = class _LiveMap extends LiveObject {
  ...
  static async createMapSetMessage(realtimeObject, objectId, key, value) {
    ...
    if (LiveCounterValueType.instanceof(value)) {
      ...
      objectData = primitiveToObjectData(value, client);
    }
  ...
  *values() {
  ...
  overrideWithObjectState(objectMessage) {
    ...
    if (!this._client.Utils.isNil(objectState.createOp)) {
      ...
      if (((_e = objectState.createOp.mapCreate) == null ? void 0 : _e.semantics) !== this._semantics) {
        throw new this._client.ErrorInfo(
          `Invalid object state: object state createOp map semantics=${(_f = objectState.createOp.mapCreate) == null ? void 0 : _f.semantics}; LiveMap semantics=${this._semantics}`,
          92e3,
          500
        );
      }
    ...
    const previousDataRef = this._dataRef;
    this._createOperationIsMerged = false;
    this._clearTimeserial = (_h = objectState.map) == null ? void 0 : _h.clearTimeserial;
    this._dataRef = this._liveMapDataFromMapEntries((_j = (_i = objectState.map) == null ? void 0 : _i.entries) != null ? _j : {});
    if (!this._client.Utils.isNil(objectState.createOp)) {
      this._mergeInitialDataFromCreateOperation(objectState.createOp, objectMessage);
    }
    const update = this._updateFromDataDiff(previousDataRef, this._dataRef);
  ...
  _mergeInitialDataFromCreateOperation(objectOperation, msg) {
    var _a, _b, _c;
    const mapCreate = (_b = objectOperation.mapCreate) != null ? _b : (_a = objectOperation.mapCreateWithObjectId) == null ? void 0 : _a._derivedFrom;
    ...
    Object.entries((_c = mapCreate.entries) != null ? _c : {}).forEach(([key, entry]) => {
  ...
  _applyMapCreate(op, msg) {
    ...
    const mapCreate = (_b = op.mapCreate) != null ? _b : (_a = op.mapCreateWithObjectId) == null ? void 0 : _a._derivedFrom;
    if (this._semantics !== (mapCreate == null ? void 0 : mapCreate.semantics)) {
      throw new this._client.ErrorInfo(
        `Cannot apply MAP_CREATE op on LiveMap objectId=${this.getObjectId()}; map's semantics=${this._semantics}, but op expected ${mapCreate == null ? void 0 : mapCreate.semantics}`,
        92e3,
        500
      );
    }
  ...
  _applyMapSet(op, opSerial, msg) {
    ...
    if (!Utils.isNil(op.value.objectId)) {
      liveData = { objectId: op.value.objectId };
      this._realtimeObject.getPool().createZeroValueObjectIfNotExists(op.value.objectId);
    } else {
      liveData = op.value;
    }
    if (existingEntry) {
   ...
   * @spec RTLM9
   */
  _canApplyMapEntryOperation(mapEntrySerial, opSerial) {
  ...
  _liveMapDataFromMapEntries(entries) {
    ...
    Object.entries(entries != null ? entries : {}).forEach(([key, entry]) => {
      ...
      const liveDataEntry = {
        ...
        tombstonedAt: entry.tombstone === true ? this._calculateTombstonedAt(
...
var LiveMapValueType = class _LiveMapValueType {
  ...
  static async createMapCreateMessage(realtimeObject, value) {
    ...
    const { mapCreate, nestedObjectsCreateMsgs } = await _LiveMapValueType._getMapCreate(realtimeObject, entries);
  ...
  static async _getMapCreate(realtimeObject, entries) {
    var _a, _b;
    const mapEntries = {};
    ...
    for (const [key, value] of Object.entries(entries != null ? entries : {})) {
      ...
      if (_LiveMapValueType.instanceof(value)) {
        ...
        objectData = primitiveToObjectData(value, realtimeObject.getClient());
      }
      mapEntries[key] = {
        data: objectData
      };
...
var mapSemanticsWireToPublic = {
  ["LWW" /* LWW */]: "lww"
};
var RestObject = class {
  ...
  async get(params) {
    ...
    const { unpacked, body } = await client.rest.Resource.get(
      client,
      this._basePath(params == null ? void 0 : params.objectId),
      headers,
      params != null ? params : {},
      null,
      true
    );
    const decoded = unpacked ? body : client.Utils.decodeBody(
      body,
      client._MsgPack,
      format
    );
    const compact = (_a = params == null ? void 0 : params.compact) != null ? _a : true;
    if (compact) {
      return decoded;
    }
    return this._decodeFullResponseNode(decoded, format);
  }
  async publish(op) {
    const client = this._channel.client;
    const format = client.options.useBinaryProtocol ? client.Utils.Format.msgpack : client.Utils.Format.json;
    const headers = client.Defaults.defaultPostHeaders(client.options, { format });
    const wireOps = Array.isArray(op) ? op.map((o) => this._constructWireOperations(o, format)) : [this._constructWireOperations(op, format)];
    client.Utils.mixin(headers, client.options.headers);
    const requestBody = client.Utils.encodeBody(wireOps, client._MsgPack, format);
    const { unpacked, body } = await client.rest.Resource.post(
      client,
      this._basePath(),
      requestBody,
      headers,
  ...
  async generateObjectId(createBody) {
    const client = this._channel.client;
    const format = client.Utils.Format.json;
    let objectType;
    let initialValueJSONString;
    if ("mapCreate" in createBody && createBody.mapCreate) {
      objectType = "map";
      const mapCreate = __spreadProps(__spreadValues({}, createBody.mapCreate), {
  ...
  _basePath(objectId) {
    return this._channel.client.rest.channelMixin.basePath(this._channel) + "/object" + (objectId ? "/" + encodeURIComponent(objectId) : "");
  }
  /**
   ...
   */
  _decodeFullResponseNode(wire, format) {
    if ("map" in wire) {
  ...
  _decodeWireRestLiveMap(wire, format) {
    var _a, _b;
    const entries = {};
    for (const [key, entry] of Object.entries((_a = wire.map.entries) != null ? _a : {})) {
      entries[key] = {
        data: this._decodeFullResponseNode(entry.data, format)
      };
    }
    const liveMap = __spreadProps(__spreadValues({}, wire), {
      objectId: wire.objectId,
      map: __spreadProps(__spreadValues({}, wire.map), {
        semantics: (_b = mapSemanticsWireToPublic[wire.map.semantics]) != null ? _b : "unknown",
        entries
      })
    });
    return liveMap;
  }
  _constructWireOperations(op, format) {
    const _a = op, { id, path, mapCreate } = _a, rest = __objRest(_a, ["id", "path", "mapCreate"]);
    const operationFields = mapCreate ? __spreadProps(__spreadValues({}, rest), {
      mapCreate: __spreadProps(__spreadValues({}, mapCreate), { semantics: encodeMapSemantics(mapCreate.semantics, this._channel.client) })
    }) : rest;
    const encoded = encodePartialObjectOperationForWire(operationFields, this._channel.client, format);
    const result = __spreadValues({}, encoded);
    if (id != null)
      result.id = id;
    if (path != null)
      result.path = path;
