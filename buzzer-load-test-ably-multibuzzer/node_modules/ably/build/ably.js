...
async function withTimeoutAsync(promise, timeout = 5e3, err = "Timeout expired") {
  ...
  return Promise.race([
    promise,
    new Promise((_resolve, reject) => Platform.Config.setTimeout(() => reject(e), timeout))
  ]);
}
...
var Push = class {
  ...
  async updateToken(token) {
    ...
    device.push.recipient = token.transportType === "apns" ? { transportType: "apns", deviceToken: token.token } : { transportType: "fcm", registrationToken: token.token };
    ...
    if (this.state === "attached" && (this._mode & flags.SUBSCRIBE) === 0) {
      ...
      if (!this._silentSubscribeWarned) {
        logger_default.logActionNoStrip(
          ...
          err.message + "; remediation=" + err.remediation + logger_default.silentFailureLogSuffix()
        );
  ...
  release(name) {
    ...
    channel.detach().catch((err) => {
      logger_default.logAction(
        ...
        logger_default.LOG_ERROR,
        "Channels.release()",
        "Error detaching channel " + name + " prior to release: " + inspectError(err)
      );
    }).then(() => {
      delete this.all[name];
    });
